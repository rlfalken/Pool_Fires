...
\bar{Y_{i}}=\frac{\bar{X_{i}}~\bar{M_{i}}}{\bar{M_{tot}}}
\end{equation}
where ${\bar{M_{i}}}$ is the molecular weight of a given species and ${\bar{M_{tot}}}$ is the average molecular weight of all detected gas species represented in the function below. 

 \begin{equation}\label{eq:Total_MW}
\bar{M_{tot}}=\sum{\bar{X_{i}}~bar{M_{i}}}
\end{equation}
...
Z=Y_{F}+\frac{\bar{M_{F}}}{x}\sum{\frac{Y_{i}}{\bar{M_{i}}}}
\end{equation} 
where $Y_{F}$, $\bar{M_{F}}$, and $x$ are the mass fraction, molecular weight, and number of carbon molecules of the parent fuel, respectively.

Considering the idealized reaction of a hydrocarbon fuel, the state reactions was derived as 

where $max(\alpha,\beta)$ and $min(\alpha,\beta)$ are operators that return the larger and smaller, respectively, of the two parameters $\alpha$ and $\beta$. 
...
\hline
\\[0.01cm]
Mean Flame Height~(\si{cm})			&	40.5~±~14.8			&	61.1~±~14.1		&	87.3~±~16.4\\	
\\[0.01cm]
...
The total mass detected in the TCD chromatogram, $m_{tot}$, was calculated from the summation of products of the moles of given species and their respective molar mass,$\bar{M_{i}}$: 
