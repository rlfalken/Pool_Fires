...
The heat of the combustion is provided by Ref.~\cite{Dippr} and reported in Table~\ref{tab:Pool_Fire_Parameters_Table}. The uncertainty of the mass burning rate is discussed in Appendix~\ref{ssec:Mass_Burning_Flux}. The uncertainty of the heat release rate and $Q^*$ is discussed in Appendix~\ref{ssec:Heat_Release_Rate}.

$Q^*$ is the normalized heat release rate of a fire and is defined as:
\begin{equation}
Q^*=\frac{\dot{Q}}{c_{p}\rho_\infty T_\infty \sqrt{g} \, D^{5/2}}
\end{equation}
...
This section presents the flame height, temperature, gas species, and soot measurements made at incremental heights along the centerline of medium-scale pool fires using various fuels such as methanol, ethanol, and acetone.
...
\\[0.01cm]							
$Q^* $~(\si{kW})						&	0.32~$\pm$~0.02		&	0.48~$\pm$~0.01		&	0.64~$\pm$~0.05		\\	
\\[0.01cm]
...
\end{equation}
where $\bar{Y}_{\rm F}$, ${W_{\rm F}}$, and $x$ are the mass fraction, molecular weight, and number of carbon atoms in the fuel molecule, respectively. Assuming ideal (i.e. no CO or soot), infinitely-fast (fuel and oxygen from the air cannot co-exist) combustion, the mass fractions of all species can be expressed as piece-wise linear ``state relations'' according to the following reaction:
\begin{align*}
...
%\Subsection{Methanol}
%\label{ssec:Meth_Measure}
%There have been many measurements made on 30 cm diameter methanol pool fires.  A list of the measurements is described in Refs. [].  The results, which may be of interest for model development and validation, is available through the MaCFP GitHub website, and include both global and local measurements: 
